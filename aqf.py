...
function_map = {
    'macd': ('get_macd', 'transform_macd', ['ticker', 'datetime']),
    'sma': ('get_sma', 'transform_sma', ['ticker', 'datetime', 'period']),
    'rsi': ('get_rsi', 'transform_rsi', ['ticker', 'datetime', 'period']),
    'intraday': ('get_intraday_data', 'transform_intraday', ['ticker', 'datetime'])
    }
    ...
    for month in months:
        for topic in topics:
            ...
            if json_data is not None:
                h_news = transf.transform_news_data(json_data,'technology')
...
for indicator in function_map:
    print(indicator)
    get_func, transform_func, cols = function_map[indicator]
    print(get_func, transform_func, cols)

    if indicator == 'intraday':
        for symbol in symbols:
            for month in months:
                json_data = getattr(client, get_func)(symbol, month)
                if json_data:
                    transformed_data = getattr(transf, transform_func)(symbol, json_data)
                    df_indicator = combine_data(df_indicator, transformed_data, subset_columns=cols)
...
if historical_needed:
    ...
    bm.write_csv(df_sma,'data/df_sma.csv')
    bm.write_csv(df_rsi,'data/df_rsi.csv')
    bm.write_csv(df_news,'data/news.csv')
